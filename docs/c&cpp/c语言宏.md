...
宏定义实现了一个调试信息输出的功能。当编译时定义了DEBUG宏时，调用DEBUG\_PRINT会将信息输出到标准错误流中；否则，DEBUG\_PRINT会被替换为空语句
...
宏可以用于调试程序，比如在程序中加入调试信息、跟踪程序执行等。这里的 <code>__FILE__</code>、<code>__LINE__</code>属于内置宏，分别代表当前文件、当前行
